function TM(nodes, links, user_input) {
    ...
    this.inputAlphabet = this.getInputAlphabet(false);
    this.tapeAlphabet = this.getInputAlphabet(true);

    this.initialState = this.getInitialState();
    this.acceptState = this.getAcceptState();
    this.rejectState = this.getRejectState();

    this.configurations = [];
    this.statesPathText = [];
    this.statesPath = [];
    this.transitionsPathText = [];
    this.transitionsPath = [];
}

TM.prototype.evaluateString = function() {
    var current_tape_index = 1;
    var current_state = this.initialState.text,
        current_transition = null,
        temp_tape = this.tape,
        state_change = false;;
    while (current_state != this.acceptState.text && current_state != this.rejectState.text) {

        for (var current_input_symbol = 1; current_input_symbol < temp_tape.length - 1; current_input_symbol++) {
            state_change = false;

            for (var transition_index = 0; transition_index < this.transitionTable.length; transition_index++) {
                current_transition = this.transitionTable[transition_index];
                ...
                if ((current_transition.start_state == current_state) && (current_transition.input_symbol == temp_tape.charAt(current_tape_index))) {
                    ...
                    this.statesPathText.push(current_state);
                    this.transitionsPathText.push(current_transition);
        ...
        if (current_state == this.acceptState.text || current_state === this.rejectState.text) {
            state_change = true;
        }

        if (!state_change) {
            current_state = this.rejectState.text;
        }
    }

    if (current_state == this.acceptState.text) {
        sweetAlert("¡¡Éxito!!", "La cadena es aceptada", "success");
    } else {
        sweetAlert("Oops...", "La cadena no es aceptada", "error");
    }

    this.statesPath = this.getStatesFromText();
    this.transitionsPath = this.getTransitionsFromText();
};
...
TM.prototype.animateMachine = function() {
    ...
    while (node_amount < this.configurations.length) {
        add_animation(this.transitionsPath[node_amount], time_amount + 1, "yellow");
        add_text_animation(this.configurations[node_amount], time_amount + 1);
        add_animation(this.transitionsPath[node_amount], time_amount + 3, "white");
        if (this.statesPath[node_amount] != undefined) {
            add_animation(this.statesPath[node_amount], time_amount + 5, "yellow", 31);
            add_animation(this.statesPath[node_amount], time_amount + 7, "white", 31);
        }
    ...
    var diff_amount = this.configurations.length - this.statesPath.length + 1;
    if (this.statesPath[node_amount - diff_amount].text == this.acceptState.text) {
        add_animation(this.statesPath[node_amount - diff_amount], time_amount, "blue", 31);
    } else {
        add_animation(this.statesPath[node_amount - diff_amount], time_amount, "red", 31);
    }
};

TM.prototype.getInputAlphabet = function(add_blank) {
    var alphabet = new Set();
    for (var index = 0; index < this.transitionTable.length; index++)
        if (!alphabet.has(this.transitionTable[index].input_symbol)) {
            alphabet.add(this.transitionTable[index].input_symbol);

            if (this.transitionTable[index].input_symbol.localeCompare("_"))
                add_blank = !add_blank
        }
...
TM.prototype.createTransitionTable = function() {
    ...
    for (var index = 0; index < this.transitions.length; index++) {
        ...
        if (!(current_transition instanceof StartLink)) {
            ...
            transitionTable.push(new_transition);
        }
    }
...
TM.prototype.validateTmStructure = function() {
    ...
    var trans_text = nodes_text = acceptState = false;
    var error_message = "VALIDACIONES DE ESTRUCTURA";

    if (acceptState = (this.acceptState == null))
    ...
    if (!trans_text || !nodes_text || acceptState) {
        //alert(error_message);
        sweetAlert("¡Alerta!", error_message, "warning");
        return false;
    }
...
TM.prototype.showUniqueStatesOnHTML = function() {
    $(".initialState").text(this.initialState.text);
    $(".acceptState").text(this.acceptState.text);
    if (this.rejectState != null)
        $(".rejectState").text(this.rejectState.text);
    else
        $(".rejectState").text("qr (default)");
};

TM.prototype.showTransitionTableOnHTML = function() {
    var transitionTable_text = "Transition table\n";
    ...
    for (var index = 0; index < this.transitionTable.length; index++) {
        ...
        transitionTable_text += "[";
        start_state_column = document.createElement("td");
        start_state_column.textContent = this.transitionTable[index].start_state;
        new_row.appendChild(start_state_column);
        transitionTable_text += this.transitionTable[index].start_state + "\t";

        input_symbol_column = document.createElement("td");
        input_symbol_column.textContent = this.transitionTable[index].input_symbol;
        new_row.appendChild(input_symbol_column);
        transitionTable_text += this.transitionTable[index].input_symbol + "\t";

        set_in_tape_column = document.createElement("td");
        set_in_tape_column.textContent = this.transitionTable[index].set_in_tape;
        new_row.appendChild(set_in_tape_column);
        transitionTable_text += this.transitionTable[index].set_in_tape + "\t";

        move_to_column = document.createElement("td");
        move_to_column.textContent = this.transitionTable[index].move_to;
        new_row.appendChild(move_to_column);
        transitionTable_text += this.transitionTable[index].move_to + "\t";

        final_state_column = document.createElement("td");
        final_state_column.textContent = this.transitionTable[index].final_state;
        new_row.appendChild(final_state_column);

        $(".transitions tbody").append(new_row);

        transitionTable_text += this.transitionTable[index].final_state + "]\n";
    }
};

TM.prototype.showAlphabetsOnHTML = function() {
    var inputAlphabet_text = Array.from(this.inputAlphabet);
    $(".inputAlphabet").text("{" + inputAlphabet_text.toString() + "}");
    $(".tapeAlphabet").text("{" + inputAlphabet_text.toString() + "}");
};

TM.prototype.getStatesFromText = function() {
    var ret_val = [];
    for (var text_index = 0; text_index < this.statesPathText.length; text_index++)
        for (var state_index = 0; state_index < this.states.length; state_index++)
            if (this.states[state_index].text === this.statesPathText[text_index]) {
...
TM.prototype.getTransitionsFromText = function() {
    var ret_val = [],
        ...
        transition_input_and_movement = null;
    for (var text_index = 0; text_index < this.transitionsPathText.length; text_index++)
        for (var trans_index = 0; trans_index < this.transitions.length; trans_index++) {
            current_transition = this.transitions[trans_index];
            current_text_trans = this.transitionsPathText[text_index];
            if (!(current_transition instanceof StartLink)) {
...
function genericValidations() {
    ...
    if (nodes.length == 0) {
        error_message += "La máquina debe de tener estados\n";
        has_errors = true;
    }
    if (!validateStartLink()) {
        error_message += "La máquina debe tener un estado inicial\n";
        has_errors = true;
    }
    if (links.length == 0) {
        error_message += "La máquina debe de tener transiciones\n";
        has_errors = true;
    } else if (!validateLinksHasGenericText()) {
        error_message += "Las transiciones, a excepción de la inicial, necesitan tener texto\n";
        has_errors = true;
    }
...
/*
L(M) = w#w
{"nodes":[{"x":409,"y":83,"text":"A","isAcceptState":false,"isRejectState":false},
          {"x":247,"y":171,"text":"B","isAcceptState":false,"isRejectState":false},
          {"x":569,"y":171,"text":"C","isAcceptState":false,"isRejectState":false},
          {"x":247,"y":335,"text":"D","isAcceptState":false,"isRejectState":false},
          {"x":569,"y":335,"text":"E","isAcceptState":false,"isRejectState":false},
          {"x":409,"y":409,"text":"F","isAcceptState":false,"isRejectState":false},
          {"x":409,"y":534,"text":"G","isAcceptState":false,"isRejectState":false},
          {"x":409,"y":205,"text":"H","isAcceptState":false,"isRejectState":false},
          {"x":409,"y":312,"text":"I","isAcceptState":true,"isRejectState":false}],
          "links":[
            {"type":"StartLink","node":0,"text":"","deltaX":0,"deltaY":-67},
            {"type":"Link","nodeA":0,"nodeB":1,"text":"0->X,R","lineAngleAdjust":0,"parallelPart":0.5,"perpendicularPart":0},
            {"type":"Link","nodeA":0,"nodeB":2,"text":"1->X,R","lineAngleAdjust":3.141592653589793,"parallelPart":0.5458823529411765,"perpendicularPart":0},
            {"type":"SelfLink","node":2,"text":"0->0,R","anchorAngle":0.20177274421866476},
            {"type":"SelfLink","node":2,"text":"1->1,R","anchorAngle":0.4327545927209293},
            {"type":"SelfLink","node":1,"text":"0->0,R","anchorAngle":2.85725145572374},
            {"type":"SelfLink","node":1,"text":"1->1,R","anchorAngle":2.580464220401414},
            {"type":"Link","nodeA":1,"nodeB":3,"text":"#->#,R","lineAngleAdjust":0,"parallelPart":0.5,"perpendicularPart":0},
            {"type":"Link","nodeA":2,"nodeB":4,"text":"#->#,R","lineAngleAdjust":3.141592653589793,"parallelPart":0.6143790849673203,"perpendicularPart":0},
            {"type":"Link","nodeA":0,"nodeB":7,"text":"#->#,R","lineAngleAdjust":0,"parallelPart":0.5,"perpendicularPart":0},
            {"type":"SelfLink","node":7,"text":"X->X,R","anchorAngle":0},
            {"type":"Link","nodeA":7,"nodeB":8,"text":"_->_,R","lineAngleAdjust":0,"parallelPart":0.5,"perpendicularPart":0},
            {"type":"SelfLink","node":3,"text":"X->X,R","anchorAngle":3.141592653589793},
            {"type":"SelfLink","node":4,"text":"X->X,R","anchorAngle":0},
            {"type":"Link","nodeA":4,"nodeB":5,"text":"1->X,L","lineAngleAdjust":0,"parallelPart":0.5,"perpendicularPart":0},
            {"type":"Link","nodeA":3,"nodeB":5,"text":"0->X,L","lineAngleAdjust":3.141592653589793,"parallelPart":0.5497283998733346,"perpendicularPart":0},
            {"type":"Link","nodeA":5,"nodeB":6,"text":"#->#,L","lineAngleAdjust":0,"parallelPart":0.5,"perpendicularPart":0},
            {"type":"SelfLink","node":5,"text":"1->1,L","anchorAngle":0.4157457564158564},
            {"type":"SelfLink","node":5,"text":"0->0,L","anchorAngle":0.1649338854081928},
            {"type":"SelfLink","node":5,"text":"X->X,L","anchorAngle":0.7427613968736926},
            {"type":"Link","nodeA":6,"nodeB":0,"text":"X->X,R","lineAngleAdjust":3.141592653589793,"parallelPart":0.7516629711751663,"perpendicularPart":-296},
            {"type":"SelfLink","node":6,"text":"0->0,L","anchorAngle":0},
            {"type":"SelfLink","node":6,"text":"1->1,L","anchorAngle":0.22513803724713607}]} 
*/
